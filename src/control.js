...
export const Elevation = (L.Control.Elevation = L.Control.extend({
  ...
  initialize: function (options) {
    ...
    if (this.options.theme)
      this.options.polylineSegments.className += " " + this.options.theme;
  ...
  loadGeoJSON: function (data) {
    this.geoJsonLayer = _.GeoJSONLoader(data, this);
  },

  /**
   * Returns layer responsible to display GeoJson data
   */
  ...
  loadLazy: function (data, opts) {
    ...
    _.waitHolder(elem).then(() => {
      opts.lazy = false;
      this.loadData(data, opts);
      this.once("eledata_loaded", () =>
        this.placeholder.parentNode.removeChild(elem)
      );
    });
  },
  ...
  onAdd: function (map) {
    ...
    let container = (this._container = _.create(
      "div",
  ...
  _initElevationDiv: function () {
    ...
    if (!eleDiv) {
      ...
      eleDiv = _.create("div", "leaflet-control elevation elevation-div", {
  ...
  _initLayer: function () {
    this._layers.on("layeradd layerremove", (e) => {
      let layer = e.layer;
  ...
  _initButton: function (container) {
    ...
    if (!this.options.detached) {
      let link = (this._button = _.create(
        "a",
  ...
  _initSummary: function (container) {
    let summary = (this._summary = new Summary({
   ...
   */
  _toggle: function () {
    if (_.hasClass(this._container, "elevation-expanded")) this._collapse();
    else this._expand();
  },
  ...
  _updateSummary: function () {
    ...
    if (this.options.downloadLink && this._downloadURL) {
      ...
      this.summaryDiv.innerHTML +=
        '<span class="download"><a href="#">' + L._("Download") + "</a></span>";
      _.select(".download a", this.summaryDiv).onclick = (e) => {
        e.preventDefault();
        this._fireEvt("eletrack_download", {
          downloadLink: this.options.downloadLink,
          confirm: _.saveFile.bind(this, this._downloadURL),
        });
      };
    }
  },
...
Elevation.addInitHook(function () {
  ...
  this.on("elepath_toggle", function (e) {
    ...
    this._chartEnabled =
      this._chart._area.selectAll("path:not(.leaflet-hidden)").nodes().length !=
      0;
    this._layers.eachLayer((l) =>
      _.toggleClass(
    ...
    this.options[optName] =
      enable && this.options[optName] == "disabled" ? "enabled" : "disabled";
  ...
  this.on("elechart_updated elechart_init", function () {
    ...
    let v = Array(Math.floor(n / 2))
      .fill(null)
      .map((d, i) => (i + 1) * 2 - (1 - Math.sign(n % 2)));
    let rev = v
      .slice()
      .reverse()
      .map((d) => -d);
    ...
    items.each((d, i, n) => {
      ...
      let path = this._chart._area
        .select('path[data-name="' + name + '"]')
        .node();
      // Bind legend click togglers
      d3.select(target).on("click", () =>
        this._fireEvt("elepath_toggle", {
          path: path,
          name: name,
          legend: target,
        })
      ...
      if (
        path &&
        optName in this.options &&
        this.options[optName] == "disabled"
      ) {
      ...
      if (path && this._chart._clipPath) {
        path.setAttribute(
          "clip-path",
          "url(#" + this._chart._clipPath.attr("id") + ")"
        );
      }
      // Adjust legend item positions
      d3.select(target).attr(
        "transform",
        "translate(" + v[i] * 55 + ", " + xAxesB * 2 + ")"
      );
    });
